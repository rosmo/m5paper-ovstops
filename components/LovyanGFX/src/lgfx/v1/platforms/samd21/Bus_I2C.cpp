...
//----------------------------------------------------------------------------
  ...
  void Bus_I2C::dc_control(bool dc)
  {
    ...
    lgfx::i2c::writeBytes(_cfg.sercom_index, (uint8_t*)(dc ? &_cfg.prefix_data : &_cfg.prefix_cmd), _cfg.prefix_len);
    _state = st;
  }
  ...
  {
    dc_control(true);
    lgfx::i2c::writeBytes(_cfg.sercom_index, (uint8_t*)&data, (bit_length >> 3));
  }
  ...
  uint32_t Bus_I2C::readData(uint_fast8_t bit_length)
  {
    beginRead();
    uint32_t res;
    i2c::readBytes(_cfg.sercom_index, reinterpret_cast<uint8_t*>(&res), bit_length >> 3);
    return res;
  }
