...
int logging_vprintf( const char *fmt, va_list l ) {
    ...
    buffer = (char *)calloc(xItemSize, sizeof(char));
    assert(buffer != NULL);

	int buffer_len = vsprintf(buffer, fmt, l);
	if (buffer_len > 0) {
        msg = (char *)calloc(xItemSize, sizeof(char));
        assert(msg != NULL);
...
{
#ifdef CONFIG_OVS_SYSLOG_SERVER
    udp_logging_init(CONFIG_OVS_SYSLOG_SERVER, 514, 1);
#endif
}